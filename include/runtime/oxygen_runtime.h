...
unsigned long long hash64(long n) {
    unsigned long long hash= clock() ^ 13 + n;
    return hash;
}
...
void new_string(string* s, const char* str ) {
    s->size = strlen(str);
...
void string_concat(string* s, const char* str) {
...
void string_reserve(string* s, unsigned int size) {
    s->data = (char*)realloc(s->data, s->capacity+size);
    s->capacity += size;
}

void string_deleter(string* s) {
    free(s->data);
    free(s);
}
